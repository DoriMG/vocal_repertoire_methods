...
data_file =  "contexts.csv"
df_loc <- read.csv(data_file, header=TRUE, stringsAsFactors=TRUE)
df_loc$percs = df_loc$percs*100
loc_order = c("group", 'both', 'solitary')
context = ggplot(df_loc, aes(y=percs, x = factor(context , loc_order) , fill=factor(context , loc_order)  )) +
  geom_bar(stat="identity")+
